...
{
    juce::ScopedNoDenormals noDenormals; // 작은값이나 0에 가까운값의 오버플로우 언더플로두 방지
    auto totalNumInputChannels  = getTotalNumInputChannels(); // 입력채널 수
    auto totalNumOutputChannels = getTotalNumOutputChannels(); // 출력채널 수
    ...
    for (auto i = totalNumInputChannels; i < totalNumOutputChannels; ++i)
        buffer.clear (i, 0, buffer.getNumSamples()); // 입력채널수보다 출력채널수가 더 많으면 입력데이터가 없는 출력 채널을 초기화한다.
    auto chainSettings = getChainSettings(apvts);
...
//    * rightChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients;
    ...
    juce::dsp::AudioBlock<float> block(buffer); // 주어진 버퍼로 AudioBlock을 생성
                                                // AudioBlock은 JUCE에서 쓰는 오디오 데이터의 블록 클래스
    
    auto leftBlock = block.getSingleChannelBlock(0); // 왼쪽 채널의 오디오 블록
    auto rightBlock = block.getSingleChannelBlock(1); // 오른쪽 채널의 오디오 블록
